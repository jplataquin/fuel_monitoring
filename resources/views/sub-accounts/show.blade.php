<x-app-layout>
    ...
    <div class="py-5" x-data="{ activeTab: 'accomplishments' }">
        <div class="container" style="max-width: 1000px;">
            ...
            <div class="card bg-dark border border-secondary border-opacity-25 rounded-3 p-4 mb-5">
                <div class="row align-items-center g-4">
                    <div class="col-md-3">
                        <h3 class="text-info small fw-bold text-uppercase tracking-widest mb-2" style="font-size: 0.75rem;">Sub-Account Name</h3>
                        <span class="h4 text-white fw-bold d-block">{{ $subAccount->display_name }}</span>
                    </div>

                    <div class="col-md-2">
                        <h3 class="text-info small fw-bold text-uppercase tracking-widest mb-2" style="font-size: 0.75rem;">Quantity</h3>
                    ...
                    <div class="col-md-2">
                        <h3 class="text-info small fw-bold text-uppercase tracking-widest mb-2" style="font-size: 0.75rem;">Progress</h3>
                        <span class="h4 text-white fw-bold font-monospace d-block">{{ number_format($subAccount->accomplishment, 2) }}%</span>
                        @if($subAccount->quantity)
                            <span class="text-secondary smaller fw-bold text-uppercase mt-1 d-block" style="font-size: 0.7rem;">Done: {{ number_format($subAccount->accomplishments()->sum('quantity'), 2) }} {{ $subAccount->unit }}</span>
                        @endif
                    </div>

                    <div class="col-md-2">
                        <h3 class="text-info small fw-bold text-uppercase tracking-widest mb-2" style="font-size: 0.75rem;">Latest Accomplishment</h3>
                        @php
                        ...
                        @if($latestAcc)
                            <span class="text-secondary smaller fw-bold text-uppercase mt-1 d-block" style="font-size: 0.7rem;">On {{ $latestAcc->date_at->format('M d, Y') }}</span>
                        @endif
                    </div>

                    <div class="col-md-3 text-md-end">
                        <h3 class="text-info small fw-bold text-uppercase tracking-widest mb-2" style="font-size: 0.75rem;">Approved Budget</h3>
                        <p class="display-6 fw-bold text-white mb-0 font-monospace">
                            {{ number_format($subAccount->budgets()->where('status', 'Approved')->sum('budget_quantity'), 2) }} <span class="h6 text-secondary text-uppercase">L</span>
                        </p>
                    </div>
                </div>
            </div>

            <!-- Terminal Tabbed Navigation -->
            <div class="d-flex border-bottom border-secondary border-opacity-25 mb-4 gap-2">
                @if(in_array(Auth::user()->role, ['administrator', 'moderator', 'budgeteer']))
                    <button @click="activeTab = 'accomplishments'" 
                            :class="activeTab === 'accomplishments' ? 'border-bottom border-3 border-success text-success fw-bold' : 'text-secondary hover-text-white'"
                ...
                @if(in_array(Auth::user()->role, ['administrator', 'budgeteer']))
                    <button @click="activeTab = 'budgets'" 
                            :class="activeTab === 'budgets' ? 'border-bottom border-3 border-info text-info fw-bold' : 'text-secondary hover-text-white'"
            ...
            @if(in_array(Auth::user()->role, ['administrator', 'moderator', 'budgeteer']))
                <div x-show="activeTab === 'accomplishments'" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100">
                    ...
                    <div class="card bg-dark border border-secondary border-opacity-25 rounded-3 p-4 mb-4">
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            <h4 class="h5 fw-bold text-white mb-0">Log Accomplishment</h4>
                            <span class="badge bg-success bg-opacity-10 text-success border border-success border-opacity-25 rounded-2 px-3 py-2 fw-bold text-uppercase small" style="font-size: 0.75rem;">Accomplishment Registry</span>
                        </div>
                        <form action="{{ route('sub-accounts.accomplishments.store', $subAccount) }}" method="POST">
                            ...
                            <div class="row g-4 mb-4">
                                <div class="col-md-4">
                                    ...
                                    <input type="number" name="quantity" id="quantity" step="0.01" min="0.01" required placeholder="0.00" class="form-control bg-dark border-secondary text-white rounded-3 p-3 focus-ring focus-ring-success @error('quantity') is-invalid @enderror" autofocus>
                                    @error('quantity')
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="date" name="date_at" id="date_at" required value="{{ date('Y-m-d') }}" max="{{ date('Y-m-d') }}" class="form-control bg-dark border-secondary text-white rounded-3 p-3 focus-ring focus-ring-success @error('date_at') is-invalid @enderror">
                                    @error('date_at')
                                        <div class="text-danger small fw-bold mt-2">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="col-md-4 d-flex align-items-end">
                                    <button type="submit" class="btn btn-success px-4 py-3 rounded-3 fw-bold small text-uppercase tracking-wider shadow-sm w-100" style="cursor: pointer;">
                        ...
                        <div class="mt-4 card bg-dark border border-secondary border-opacity-25 rounded-3 overflow-hidden">
                ...
                <div x-show="activeTab === 'budgets'" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100">
                    ...
                    <div class="card bg-dark border border-secondary border-opacity-25 rounded-3 p-4 mb-4">
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            ...
                            <span class="badge bg-info bg-opacity-10 text-info border border-info border-opacity-25 rounded-2 px-3 py-2 fw-bold text-uppercase small" style="font-size: 0.75rem;">Pending Approval</span>
                        </div>
                        <form action="{{ route('account-budgets.store') }}" method="POST">
                            ...
                            <div class="row g-4 mb-4">
                                <div class="col-md-6">
                                    ...
                                    <input type="number" name="budget_quantity" id="budget_quantity" step="0.01" required placeholder="0.00" class="form-control bg-dark border-secondary text-white rounded-3 p-3 focus-ring focus-ring-info @error('budget_quantity') is-invalid @enderror">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="remarks" id="remarks" placeholder="Optional notes..." class="form-control bg-dark border-secondary text-white rounded-3 p-3 focus-ring focus-ring-info @error('remarks') is-invalid @enderror">
                                    @error('remarks')
                                        <div class="text-danger small fw-bold mt-2">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="d-flex justify-content-end pt-1 pb-3">
                                <button type="submit" class="btn btn-info px-4 py-2 rounded-3 fw-bold small text-uppercase tracking-wider shadow-sm" style="cursor: pointer;">
                                    Submit for Approval
                                </button>
                            </div>
                        </form>
                                            ...
                                            @forelse($subAccount->budgets()->orderBy('created_at', 'desc')->get() as $budget)
                                                <tr>
                                                    <td class="px-4 py-3 align-middle">
                                                        ...
                                                        <span class="d-block text-secondary smaller fw-bold text-uppercase" style="font-size: 0.7rem;">{{ $budget->created_at->format('h:i A') }}</span>
                                                    </td>
                                                    ...
                                                    <td class="px-4 py-3 align-middle text-center">
                                                        <span class="badge rounded-2 fw-bold text-uppercase smaller px-3 py-2
