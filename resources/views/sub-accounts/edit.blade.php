            ...
            <div class="card bg-dark border-secondary shadow-lg rounded-4 p-4">
                <form method="POST" action="{{ route('sub-accounts.update', $subAccount) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input id="name" name="name" type="text" class="form-control bg-dark border-secondary text-white rounded-3 p-3 focus-ring focus-ring-primary" value="{{ old('name', $subAccount->name) }}" required autofocus pattern="^[^:]+$" title="Colons (:) are not allowed in the sub-account name">
                        @error('name')
                    ...
                    <div class="row g-4 mb-4">
                        <div class="col-md-6">
                            <label for="quantity" class="form-label text-secondary small fw-bold text-uppercase tracking-wider">Quantity</label>
