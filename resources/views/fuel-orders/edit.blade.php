            ...
            <div class="col-md-10 col-lg-8">
                <div class="card bg-dark border-secondary border-opacity-25 shadow-lg rounded-4 overflow-hidden">
                    <div class="card-body p-4 p-md-5">
                        <form method="POST" action="{{ route('fuel-orders.update', $fuelOrder) }}">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="text-center mb-5 pb-5 border-bottom border-secondary border-opacity-25">
                                ...
                                <div class="d-inline-flex align-items-center gap-3 p-3 bg-secondary bg-opacity-10 rounded-3 border border-secondary border-opacity-25 shadow-sm">
                                    <label for="status" class="text-secondary fw-bold small text-uppercase tracking-wider mb-0">Status:</label>
                                    <select name="status" id="status" class="form-select form-select-lg bg-dark text-primary border-primary border-opacity-50 fw-black px-4 rounded-3" style="width: 200px;">
                                        <option value="PEND" {{ old('status', $fuelOrder->status) === 'PEND' ? 'selected' : '' }}>PEND</option>
                                        <option value="DONE" {{ old('status', $fuelOrder->status) === 'DONE' ? 'selected' : '' }}>DONE</option>
                                        <option value="VOID" {{ old('status', $fuelOrder->status) === 'VOID' ? 'selected' : '' }}>VOID</option>
                                    </select>
                                </div>
                                @error('status')
                            ...
                            <div class="row g-3 mb-5">
                                <div class="col-md-4">
                                    <div class="bg-secondary bg-opacity-10 rounded-3 p-3 border border-secondary border-opacity-10 text-center shadow-sm">
                                ...
                                <div class="col-md-4">
                                    <div class="bg-primary bg-opacity-10 rounded-3 p-3 border border-primary border-opacity-10 text-center shadow-sm">
                                        <h4 class="small fw-bold text-primary text-uppercase tracking-wider mb-2">Consumed Fuel (L)</h4>
                            ...
                            <div class="bg-secondary bg-opacity-10 rounded-4 p-4 p-md-5 mb-5 shadow-inner border border-secondary border-opacity-25">
                                <div class="mb-4">
                                    <label for="say_quantity" class="form-label h6 fw-bold text-secondary text-uppercase tracking-wider mb-2">Say Fuel Quantity (Approved) <span class="text-danger">*</span></label>
                                    <div class="input-group input-group-lg shadow-sm">
                                        <input type="number" step="0.01" name="say_quantity" id="say_quantity" value="{{ old('say_quantity', $fuelOrder->say_quantity) }}" class="form-control bg-dark text-primary border-secondary border-opacity-50 fw-black px-4 font-monospace" required placeholder="0.00">
