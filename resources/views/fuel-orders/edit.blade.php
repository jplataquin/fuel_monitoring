    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                 ...
                 <div class="bg-white rounded-[28px] shadow-2xl border border-gray-100 p-10 md:p-14">
                    ...
                    <div class="text-center mb-10 pb-10 border-b-2 border-gray-200">
                        ...
                        <div class="mt-4 flex justify-center items-center gap-3 p-4 bg-[#2D2930] rounded-2xl inline-flex border border-[#49454F]">
                            <label for="status" class="text-[#CAC4D0] font-bold text-lg uppercase tracking-wider">Status:</label>
                            <div class="relative">
                                <select name="status" id="status" class="w-64 appearance-none rounded-xl border-2 border-[#D0BCFF]/50 bg-[#D0BCFF]/10 shadow-inner focus:border-[#D0BCFF] focus:ring focus:ring-[#D0BCFF]/50 font-black text-[#D0BCFF] pl-6 pr-12 py-3 text-xl cursor-pointer transition-all hover:bg-[#D0BCFF]/20 outline-none">
                                    <option value="PEND" class="bg-[#1C1B1F] text-[#E6E1E5]" {{ old('status', $fuelOrder->status) === 'PEND' ? 'selected' : '' }}>PEND</option>
                                    <option value="DONE" class="bg-[#1C1B1F] text-[#E6E1E5]" {{ old('status', $fuelOrder->status) === 'DONE' ? 'selected' : '' }}>DONE</option>
                                    <option value="VOID" class="bg-[#1C1B1F] text-[#E6E1E5]" {{ old('status', $fuelOrder->status) === 'VOID' ? 'selected' : '' }}>VOID</option>
                                </select>
                                <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-4 text-[#D0BCFF]">
                ...
                <div class="bg-gray-900 rounded-2xl p-8 mb-12">
                    <div class="mb-6">
                        <label for="say_quantity" class="block text-gray-300 font-bold text-lg mb-2">Say Fuel Quantity (Approved) <span class="text-red-500">*</span></label>
                        <div class="relative">
                            <input type="number" step="0.01" name="say_quantity" id="say_quantity" value="{{ old('say_quantity', $fuelOrder->say_quantity) }}" class="mt-1 block w-full rounded-xl border-gray-600 bg-gray-800 text-indigo-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-2xl font-black px-4 py-3 font-mono pr-12" required placeholder="0.00">
